...
int main(void)
{
    ...
    unsigned char *img = stbi_load("D:/J/1_study/5_OpenGL_basics/learnopengl/LearnOpenGL/resources/textures/container.jpg", &width, &height, &channel, 0);
    ...
    unsigned char* img2 = stbi_load("D:/J/1_study/5_OpenGL_basics/learnopengl/LearnOpenGL/resources/textures/awesomeface.png", &width, &height, &channel, 0);
    if (img == NULL)
    {
        std::cout << "Image Load Error!" << std::endl;
        glfwTerminate();
    ...
    // Shader
    Shader myShader("shader_vertex.glsl", "shader_fragment.glsl");
    myShader.use();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        transMatrix = glm::rotate(transMatrix, glm::radians(90 * t), glm::vec3(0.0, 0.0, 1.0));
        transMatrix = glm::translate(transMatrix, glm::vec3(0.5, -0.5, 0.0));
        transMatrix = glm::scale(transMatrix, glm::vec3(0.5, 0.5, 0.5));
        myShader.setMat4("Trans", transMatrix);

        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        glm::mat4 transMatrix2(1.0f);
        transMatrix2 = glm::translate(transMatrix2, glm::vec3(-0.5, 0.5, 0.0));
        transMatrix2 = glm::rotate(transMatrix2, glm::radians(-90 * t), glm::vec3(0.0, 0.0, 1.0));
        transMatrix2 = glm::scale(transMatrix2, glm::vec3(0.5, 0.5, 0.5));
        myShader.setMat4("Trans", transMatrix2);
